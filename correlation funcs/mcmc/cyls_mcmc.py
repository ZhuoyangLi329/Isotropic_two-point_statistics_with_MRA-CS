...
for n in range(nhs):
    ...
    dataori=np.fromfile('/data1/quijote/lzy_pos/res_cyls/4/qjthalocyls'+str(n)+'.txt',dtype=np.float64)
...
def loglike_combine(theta):
    ...
    ipara=[f,b]
    for i in range(10):
        th_vector[i]=pcf_cyls_multipole(rlist[i],0,ipara)
        th_vector[i+10]=pcf_cyls_multipole(rlist[i],2,ipara)
